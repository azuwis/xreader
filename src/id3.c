...
{
	fread(tagValue, sizeof(char), tagLength, fp);
	tagValue[tagLength - 1] = '\0';
}
...
int ID3v2TagSize(const char *mp3path)
{
	...
	size =
		(((size & 0x7f000000) >> 3) | ((size & 0x7f0000) >> 2) |
...
int ParseID3v2(const char *mp3path, struct ID3Tag *id3tag)
{
	...
	if (*id3tag->ID3Title == 0) {
		...
		if (*strrchr(id3tag->ID3Title, '.') != 0)
			*strrchr(id3tag->ID3Title, '.') = 0;
	}
	return 0;
}
...
int ParseID3v1(const char *mp3path, struct ID3Tag *id3tag)
{
	...
	if (strstr(id3buffer, "TAG") != NULL) {
		...
		if (((int) id3tag->ID3GenreCode[0] >=
			 0) & ((int) id3tag->ID3GenreCode[0] < genreNumber)) {
