...
static int mp3_seek_seconds_offset_brute(double npt)
{
	...
	if (data.use_buffer)
		...
		xrIoLseek(data.fd, mp3info.frameoff[pos], PSP_SEEK_SET);
	...
	do {
		...
		if (stream.buffer == NULL || stream.error == MAD_ERROR_BUFLEN) {
			...
			if (data.use_buffer)
				...
				bufsize = xrIoRead(data.fd, read_start, read_size);
...
static int mp3_seek_seconds_offset(double npt)
{
	...
	if (data.use_buffer)
		...
		xrIoLseek(data.fd, new_pos, PSP_SEEK_SET);
		...
		if (avail_frame >= snd_buf_frame_size) {
			...
			do {
				...
				if (data.use_buffer)
					...
					ret = xrIoRead(data.fd, memp3_input_buf, frame_size);
...
static int mp3_load(const char *spath, const char *lpath)
{
	...
	data.size = xrIoLseek(data.fd, 0, PSP_SEEK_END);
