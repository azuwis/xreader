...
static win_menu_predraw_data g_predraw;
						 ...
						 dword max_height)
{
	...
	disp_putstring(242, 129, COLOR_WHITE, (const byte *) number);
#else
	disp_putstring(248, 129, COLOR_WHITE, (const byte *) _("不支持"));
#endif
}
						   ...
						   dword max_height)
{
	...
	int right =
		g_predraw.x + 4 + DISP_FONTSIZE / 2 * g_predraw.max_item_len / 2;
	...
	disp_fillrect(left + 1, upper + 1,
				  right - 1, bottom - 1,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(left + DISP_FONTSIZE * 2, upper + 1,
				   COLOR_WHITE, (const byte *) _("字体设置"));
...
dword scene_fontsel(dword * selidx)
{
	...
	for (i = 0; i < NELEMS(item); i++) {
		...
		item[i].width = item[i].width > 12 ? item[i].width : 12;
		item[i].selected = false;
		item[i].icolor = config.menutextcolor;
		item[i].selicolor = config.selicolor;
		item[i].selrcolor =
	...
	static win_menu_predraw_data prev;
						   ...
						   dword max_height)
{
	...
	disp_putstring(238 - DISP_FONTSIZE * 3, 123 - 5 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) _("保存文件位置"));
						   ...
						   dword max_height)
{
	...
	disp_fillrect(238 - DISP_FONTSIZE * 10, 123 - 5 * DISP_FONTSIZE,
				  ...
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(238 - DISP_FONTSIZE * 3, 123 - 5 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) _("读取文件位置"));
	disp_line(238 - DISP_FONTSIZE * 10, 123 - 4 * DISP_FONTSIZE,
			  240 + DISP_FONTSIZE * 10, 123 - 4 * DISP_FONTSIZE, COLOR_WHITE);
}
...
//  g_predraw.x - (DISP_FONTSIZE / 2) * g_predraw.max_item_len / 2 + 2 + max_width * (DISP_FONTSIZE / 2) + 1;
//  g_predraw.x + 4 + DISP_FONTSIZE / 2 * g_predraw.max_item_len / 2;
...
//  int right = g_predraw.x + 4 + DISP_FONTSIZE / 2 * g_predraw.max_item_len / 2;
	int upper = g_predraw.upper - (DISP_FONTSIZE + 1) - 1;
	int bottom =
		g_predraw.upper + 2 + 1 + DISP_FONTSIZE + (max_height - 1) * (1 +
																	  DISP_FONTSIZE
	...
	disp_fillrect(left + 1, upper + 1, right - 1, upper + DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(left + 1 + DISP_FONTSIZE,
...
#endif
	for (i = 0; i < NELEMS(item); i++) {
		item[i].width = strlen(item[i].name) > 12 ? strlen(item[i].name) : 12;
		item[i].selected = false;
		item[i].icolor = config.menutextcolor;
		item[i].selicolor = config.selicolor;
		item[i].selrcolor =
