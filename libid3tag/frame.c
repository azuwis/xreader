...
int id3_frame_validid(char const *id)
{
	return id &&
		valid_idchar(id[0]) &&
		valid_idchar(id[1]) && valid_idchar(id[2]) && valid_idchar(id[3]);
}
...
struct id3_frame *id3_frame_new(char const *id)
{
	...
	frame =
		malloc(sizeof(*frame) + frametype->nfields * sizeof(*frame->fields));
	if (frame) {
		frame->id[0] = id[0];
		frame->id[1] = id[1];
		frame->id[2] = id[2];
				   ...
				   id3_byte_t const *data, id3_length_t length)
{
	...
	for (i = 0; i < frame->nfields; ++i) {
		if (id3_field_parse(&frame->fields[i], &data, end - data, &encoding) ==
			-1)
			return -1;
	}
							  ...
							  id3_byte_t ** ptr, int options)
{
	...
	if (!(flags & ID3_FRAME_FLAG_ENCRYPTION)) {
		...
		if (options & ID3_TAG_OPTION_COMPRESSION)
			flags |=
				ID3_FRAME_FLAG_COMPRESSION | ID3_FRAME_FLAG_DATALENGTHINDICATOR;
	}
