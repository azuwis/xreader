...
extern bool mp3info_read(p_mp3info info, int fd)
{
	...
	while((end = sceIoRead(fd, &buf[2], 65536)) > 0)
	{
		...
		{
			int brate = 0;
			if(buf[off] == 0xFF && (buf[off + 1] & 0xE0) == 0xE0 && (size = calc_framesize(buf[off + 1], buf[off + 2], &info->level, &brate, &info->samplerate)) > 0)
			{
				br += brate;
				if(info->framecount >= 0)
				{
					if(info->framecount == 0)
						...
						info->frameoff = realloc(info->frameoff, sizeof(dword) * (info->framecount + 1024));
					if(info->frameoff == NULL)
						info->framecount = -1;
					else
						info->frameoff[info->framecount ++] = dcount * 65536 + off;
				}
				off += size;
			}
			else if(buf[off + 1] == 'D' && ((buf[off] == 'I' && buf[off + 2] == '3') || (buf[off] == '3' && buf[off + 2] == 'I')))
			{
				...
				off += 6;
				int id3size = ((dword)buf[off] << 21) + ((dword)buf[off + 1] << 14) + ((dword)buf[off + 2] << 7) + (dword)buf[off + 3] + 4;
				if(info->artist[0] == 0 || info->title[0] == 0)
				{
					...
					while(off2 < end2)
					{
						...
						if(off2 + 4 < end2)
							break;
						int tsize = (int)(((dword)buf[off2] << 21) + ((dword)buf[off2 + 1] << 14) + ((dword)buf[off2 + 2] << 7) + (dword)buf[off2 + 3]);
						if(off2 + 6 + tsize < end2)
							break;
						if((buf[off2 + 4] & 0x60) > 0)
							off2 += 6;
						else if(info->artist[0] == 0 && buf[off2 - 4] == 'T' && buf[off2 - 3] == 'P' && buf[off2 - 2] == 'E' && buf[off2 - 1] == '1')
						{
							off2 += 6;
							read_id3tag(&buf[off2], (byte *)info->artist, tsize);
						}
						else if(info->title[0] == 0 && buf[off2 - 4] == 'T' && buf[off2 - 3] == 'I' && buf[off2 - 2] == 'T' && buf[off2 - 1] == '2')
						{
							off2 += 6;
							read_id3tag(&buf[off2], (byte *)info->title, tsize);
						}
						else
							off2 += 6;
						off2 += tsize;
	...
	if((dcount > 1 || off >= 128) && (info->artist[0] == 0 || info->title[0] == 0))
	{
