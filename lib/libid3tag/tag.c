				...
				case 4:
					{
						...
						/* verify extended header size */
						{
							...
							while (bytes--) {
								for (ehflags = id3_parse_uint(&flagsptr, 1);
									 ehflags; ehflags = (ehflags << 1) & 0xff) {
									if (ehflags & 0x80) {
										if (dataptr == ehend)
											goto fail;
										datalen = id3_parse_uint(&dataptr, 1);
										if (datalen > 0x7f
											|| datalen > ehend - dataptr)
											goto fail;
										dataptr += datalen;
									}
								}
						...
						if (tag->
							extendedflags & ID3_TAG_EXTENDEDFLAG_TAGISANUPDATE)
						{
...
static id3_length_t v1_render(struct id3_tag const *tag, id3_byte_t * buffer)
{
	...
	if (frame) {
		...
		for (i = 0; i < nstrings; ++i) {
			genre =
				id3_genre_number(id3_field_getstrings(&frame->fields[1], i));
			if (genre != -1)
				break;
		}
