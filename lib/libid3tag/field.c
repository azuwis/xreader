		...
		case ID3_FIELD_TYPE_LATIN1LIST:
			{
				...
				while (end - *ptr > 0) {
					...
					field->latin1list.strings[field->latin1list.nstrings++] =
						latin1;
				}
			}
			break;
		...
		case ID3_FIELD_TYPE_STRINGLIST:
			{
				...
				while (end - *ptr > 0) {
					...
					field->stringlist.strings[field->stringlist.nstrings++] =
						ucs4;
				}
			}
			break;
							  ...
							  int terminate)
{
	...
	switch (field->type) {
		...
		case ID3_FIELD_TYPE_STRINGLIST:
			...
			for (i = 0; i < field->stringlist.nstrings; ++i) {
				size +=
					id3_render_string(ptr, field->stringlist.strings[i],
									  *encoding,
...
int id3_field_setframeid(union id3_field *field, char const *id)
{
	...
	if (field->type != ID3_FIELD_TYPE_FRAMEID || !id3_frame_validid(id))
		return -1;
