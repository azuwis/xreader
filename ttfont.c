...
extern void * ttf_open(const char * filename, int size, byte ** cbuffer)
{
	...
	if(FT_New_Memory_Face(ttf->library, ttf->buf, ttf->buflen, 0, &ttf->face) != 0)
	{
...
extern void * ttf_open_buffer(const byte * buf, int len, int size, byte ** cbuffer)
{
	p_ttf ttf;
	if((ttf = (p_ttf)calloc(1, sizeof(t_ttf))) == NULL)
	{
...
extern void * ttf_open_ascii_buffer(const byte * buf, int len, int size, byte ** ebuffer)
{
	p_ttf ttf;
	if((ttf = (p_ttf)calloc(1, sizeof(t_ttf))) == NULL)
	{
...
extern void ttf_cache(void * ttfh, const byte * s, byte * zslot)
{
	...
	if(FT_Load_Char(ttf->face, u, FT_LOAD_RENDER | FT_LOAD_TARGET_MONO) == 0 && ttf->face->glyph->bitmap.buffer != NULL)
	{
		memset(zslot, 0, ttf->bpr * ttf->size);
		int sz = ttf->face->glyph->bitmap.rows;
		int bpr2 = min(ttf->face->glyph->bitmap.pitch, ttf->bpr * 8 - ttf->face->glyph->bitmap_left);
		int m, n, y = ttf->size - ttf->face->glyph->bitmap_top - (ttf->size + 8) / 9;
		if(y + sz > ttf->size)
			y = ttf->size - sz;
		if(y < 0)
			y = 0;
		...
		*width = maxx;
		for(m = 0; m < sz; m ++)
		{
			...
			for(n = 0; n < bpr2; n ++)
			{
				...
				{
					if(((*p) & e) > 0)
						zslot[y * ttf->bpr + ((x + sx) >> 3)] |= (0x80 >> ((x + sx) & 0x07));
					x ++;
...
extern bool ttf_cache_ascii(const char * filename, int size, byte ** ebuffer, byte * ewidth)
{
...
extern bool ttf_cache_ascii_buffer(const byte * buffer, int len, int size, byte ** ebuffer, byte * ewidth)
{
