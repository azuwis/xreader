...
static void patch_setbrightness(void)
{
	...
	if (orig_sceDisplaySetBrightess != NULL) {
		...
		if (pMod != NULL) {
			if(sceKernelDevkitVersion() <= 0x03070110)
				t = (u32*) libsFindImportAddrByNid(pMod->modid, "sceDisplay_driver", 0x9E3C6DC6);
			else if (sceKernelDevkitVersion() > 0x03070110)
				t = (u32*) libsFindImportAddrByNid(pMod->modid, "sceDisplay_driver", 0x267BF9F7);
		}
		if (t != NULL) {
		...
		if (pMod != NULL) {
			t = (u32*) libsFindImportAddrByNid(pMod->modid, "sceDisplay_driver", 0xE55F0D50);
		}
		if (t != NULL) {
...
int module_start(SceSize args, void *argp)
{
	...
	if(sceKernelDevkitVersion() < 0x03070110)
		...
		ret = sceKernelRegisterDefaultExceptionHandler371((void *)_pspDebugExceptionHandler);
