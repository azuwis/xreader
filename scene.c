								  ...
								  dword * index)
{
	switch (key) {
		case (PSP_CTRL_SELECT | PSP_CTRL_START):
			if (win_msgbox
				(getmsgbyid(EXIT_PROMPT), getmsgbyid(YES), getmsgbyid(NO),
				 COLOR_WHITE, COLOR_WHITE, config.msgbcolor)) {
				scene_exit();
				return win_menu_op_continue;
			}
		...
		case PSP_CTRL_CIRCLE:
			...
			disp_putstring(240 - DISP_FONTSIZE * 3, 136 - DISP_FONTSIZE / 2,
						   COLOR_WHITE,
						   (const byte *) getmsgbyid(BUTTON_PRESS_PROMPT));
			disp_flip();
			dword key, key2;
			SceCtrlData ctl;
						  ...
						  dword max_height)
{
	...
	disp_rectangle(239 - DISP_FONTSIZE * 10, 128 - 7 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 10, 145 + 7 * DISP_FONTSIZE,
				   COLOR_WHITE);
	...
	disp_fillrect(240 - DISP_FONTSIZE * 10, 127 - 6 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 10, 144 + 7 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 5, 129 - 7 * DISP_FONTSIZE,
				   COLOR_WHITE,
				   (const byte *) getmsgbyid(BUTTON_SETTING_PROMPT));
...
dword scene_txtkey(dword * selidx)
{
	...
	while ((index =
			win_menu(240 - DISP_FONTSIZE * 10, 130 - 6 * DISP_FONTSIZE, 8,
					 NELEMS(item), item, NELEMS(item), 0, 0,
						 ...
						 dword max_height)
{
	...
	disp_rectangle(239 - DISP_FONTSIZE * 10, 128 - 7 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 10, 139 + DISP_FONTSIZE, COLOR_WHITE);
	disp_fillrect(240 - DISP_FONTSIZE * 10, 129 - 7 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 10, 128 - 6 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_fillrect(240 - DISP_FONTSIZE * 10, 127 - 6 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 10, 138 + DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 5, 129 - 7 * DISP_FONTSIZE,
				   COLOR_WHITE,
				   (const byte *) getmsgbyid(BUTTON_SETTING_PROMPT));
							...
							dword max_height)
{
	...
	disp_rectangle(239 - DISP_FONTSIZE * 6, 121 - 6 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 6, 135 + 5 * DISP_FONTSIZE,
				   COLOR_WHITE);
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 6, 121 - 5 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 6 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(IMG_OPT));
	...
	disp_fillrect(241, 123 - 5 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6,
				  134 + 5 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 + DISP_FONTSIZE, 124 - 5 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   bicubic ? getmsgbyid(IMG_OPT_CUBE) :
								   getmsgbyid(IMG_OPT_LINEAR)));
	memset(number, ' ', 4);
	...
	disp_putstring(242, 127 - 2 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) number);
	disp_putstring(240 + DISP_FONTSIZE, 128 - DISP_FONTSIZE, COLOR_WHITE,
	...
	disp_putstring(240 + DISP_FONTSIZE, 129 + DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) conf_get_thumbname(config.thumb));
	char infomsg[80];

	SPRINTF_S(infomsg, "%d%%", config.imgbrightness);
	disp_putstring(240 + DISP_FONTSIZE, 130 + 2 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) infomsg);
	disp_putstring(240 + DISP_FONTSIZE, 131 + 3 * DISP_FONTSIZE, COLOR_WHITE,
				   config.
				   img_enable_analog ? (const byte *) getmsgbyid(YES) : (const
																		 byte *)
				   getmsgbyid(NO));
	disp_putstring(240 + DISP_FONTSIZE, 132 + 4 * DISP_FONTSIZE, COLOR_WHITE,
				   config.
				   load_exif ? (const byte *) getmsgbyid(YES) : (const byte *)
				   getmsgbyid(NO));
}
...
dword scene_ioptions(dword * selidx)
{
	...
	while (win_menu
		   (240 - DISP_FONTSIZE * 6, 123 - 5 * DISP_FONTSIZE, 12, NELEMS(item),
			item, NELEMS(item), 0, 0,
			config.usedyncolor ? GetBGColorByTime() : config.menubcolor, true,
			scene_ioptions_predraw, NULL, scene_ioptions_menucb) != INVALID);
	return 0;
}
#endif
						 ...
						 dword max_height)
{
	...
	disp_rectangle(239 - DISP_FONTSIZE * 6, 120 - 7 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 6 + pad, 131 + DISP_FONTSIZE,
				   COLOR_WHITE);
	disp_fillrect(240 - DISP_FONTSIZE * 6, 121 - 7 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 6 + pad, 120 - 6 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 121 - 7 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(TEXT_COLOR_OPT));
	disp_line(240 - DISP_FONTSIZE * 6, 121 - 6 * DISP_FONTSIZE,
			  239 + DISP_FONTSIZE * 6 + pad, 121 - 6 * DISP_FONTSIZE,
			  COLOR_WHITE);
	disp_fillrect(241, 122 - 6 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6 + pad,
				  130 + DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_fillrect(280, 127 - 6 * DISP_FONTSIZE, 310, 157 - 6 * DISP_FONTSIZE,
				  config.forecolor);
	disp_fillrect(280, 130 - 3 * DISP_FONTSIZE, 310, 160 - 3 * DISP_FONTSIZE,
				  config.bgcolor);
	memset(number, ' ', 4);
	utils_dword2string(RGB_R(config.forecolor), number, 4);
	disp_putstring(242, 123 - 6 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) number);
	memset(number, ' ', 4);
	...
	disp_putstring(242, 126 - 3 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) number);
	memset(number, ' ', 4);
	...
	disp_putstring(242, 128 - DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) number);
	memset(number, ' ', 4);
#ifdef ENABLE_BG
	...
	disp_putstring(242, 129, COLOR_WHITE, (const byte *) number);
#else
	disp_putstring(248, 129, COLOR_WHITE,
				   (const byte *) getmsgbyid(NO_SUPPORT));
#endif
}
...
dword scene_color(dword * selidx)
{
	...
	SPRINTF_S(infomsg, "%s：%s", getmsgbyid(FONT_COLOR),
			  getmsgbyid(FONT_COLOR_RED));
	...
	SPRINTF_S(infomsg, "%s：%s", getmsgbyid(FONT_BACK_COLOR),
			  getmsgbyid(FONT_COLOR_RED));
	STRCPY_S(item[3].name, infomsg);
#ifdef ENABLE_BG
	SPRINTF_S(infomsg, "(%s)  %s", getmsgbyid(FONT_COLOR_GRAY),
			  getmsgbyid(FONT_COLOR_GREEN));
...
#endif
	while (win_menu
		   (240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE, 12, NELEMS(item),
			item, NELEMS(item), 0, 0,
			config.usedyncolor ? GetBGColorByTime() : config.menubcolor, true,
							...
							dword max_height)
{
	...
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 7 * DISP_FONTSIZE,
				  ...
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 7 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(READ_OPTION));
	...
	disp_putstring(242 + DISP_FONTSIZE, 126 - 4 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) ((config.vertread == 3) ?
								   getmsgbyid(REVERSAL_DIRECT) : (config.
																  vertread ==
																  2) ?
								   getmsgbyid(LEFT_DIRECT) : (config.vertread ==
															  1 ?
															  getmsgbyid
															  (RIGHT_DIRECT) :
															  getmsgbyid
...
{
	disp_rectangle(239 - DISP_FONTSIZE * 6, 121 - 6 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 6, 126 - 4 * DISP_FONTSIZE,
				   COLOR_WHITE);
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 6, 121 - 5 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 6 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(OPERATION_SETTING));
	disp_line(240 - DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE,
			  239 + DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE, COLOR_WHITE);
	disp_fillrect(241, 123 - 5 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6,
				  125 - 4 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
#ifdef ENABLE_HPRM
	disp_putstring(242 + DISP_FONTSIZE, 124 - 5 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   hprmctrl ? getmsgbyid(CONTROL_PAGE) :
								   getmsgbyid(CONTROL_MUSIC)));
#else
	disp_putstring(242 + DISP_FONTSIZE, 124 - 5 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) getmsgbyid(NO_SUPPORT));
#endif
						   ...
						   dword max_height)
{
	...
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE,
				  ...
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 6 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(FONT_SETTING));
	disp_line(240 - DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE,
			  239 + DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE, COLOR_WHITE);
	disp_fillrect(241, 123 - 5 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6,
				  130 + 1 * DISP_FONTSIZE,
							...
							dword max_height)
{
	...
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE,
				  ...
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 6 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(MUSIC_SETTING));
	disp_line(240 - DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE,
			  239 + DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE, COLOR_WHITE);
	disp_fillrect(241, 123 - 5 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6,
				  127 - 2 * DISP_FONTSIZE,
	...
	disp_putstring(242, 125 - 4 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) number);
	disp_putstring(242 + DISP_FONTSIZE, 126 - 3 * DISP_FONTSIZE, COLOR_WHITE,
...
{
	disp_rectangle(239 - DISP_FONTSIZE * 6, 121 - 6 * DISP_FONTSIZE,
				   240 + DISP_FONTSIZE * 6, 137 + 7 * DISP_FONTSIZE,
				   COLOR_WHITE);
	disp_fillrect(240 - DISP_FONTSIZE * 6, 122 - 6 * DISP_FONTSIZE,
				  239 + DISP_FONTSIZE * 6, 122 - 4 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(240 - DISP_FONTSIZE * 2, 122 - 6 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(SYSTEM_OPTION));
	disp_line(240 - DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE,
			  239 + DISP_FONTSIZE * 6, 122 - 5 * DISP_FONTSIZE, COLOR_WHITE);
	disp_fillrect(241, 123 - 5 * DISP_FONTSIZE, 239 + DISP_FONTSIZE * 6,
				  136 + 7 * DISP_FONTSIZE,
	...
	disp_putstring(242 + DISP_FONTSIZE, 124 - 5 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   showhidden ? getmsgbyid(DISPLAY_OPTION) :
								   getmsgbyid(HIDDEN_OPTION)));
	disp_putstring(242 + DISP_FONTSIZE, 125 - 4 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   showunknown ? getmsgbyid(DISPLAY_OPTION) :
								   getmsgbyid(HIDDEN_OPTION)));
	disp_putstring(242 + DISP_FONTSIZE, 126 - 3 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   showfinfo ? getmsgbyid(DISPLAY_OPTION) :
								   getmsgbyid(HIDDEN_OPTION)));
	disp_putstring(242 + DISP_FONTSIZE, 127 - 2 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   allowdelete ? getmsgbyid(YES) :
								   getmsgbyid(NO)));
	disp_putstring(242 + DISP_FONTSIZE, 128 - DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) conf_get_arrangename(config.arrange));
#ifdef ENABLE_USB
	disp_putstring(242 + DISP_FONTSIZE, 129, COLOR_WHITE,
				   (const byte *) (config.
								   enableusb ? getmsgbyid(YES) :
								   getmsgbyid(NO)));
#else
	disp_putstring(242 + DISP_FONTSIZE, 129, COLOR_WHITE,
				   (const byte *) getmsgbyid(NO_SUPPORT));
	...
	disp_putstring(242 + DISP_FONTSIZE, 130 + 1 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) infomsg);
	SPRINTF_S(infomsg, "%d/%d", freq_list[config.freqs[0]][0],
			  freq_list[config.freqs[0]][1]);
	disp_putstring(242 + DISP_FONTSIZE, 131 + 2 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) infomsg);
	SPRINTF_S(infomsg, "%d/%d", freq_list[config.freqs[1]][0],
			  freq_list[config.freqs[1]][1]);
	disp_putstring(242 + DISP_FONTSIZE, 132 + 3 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) infomsg);
	SPRINTF_S(infomsg, "%d/%d", freq_list[config.freqs[2]][0],
			  freq_list[config.freqs[2]][1]);
	disp_putstring(242 + DISP_FONTSIZE, 133 + 4 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) infomsg);
	disp_putstring(242 + DISP_FONTSIZE, 134 + 5 * DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) (config.
								   dis_scrsave ? getmsgbyid(YES) :
								   getmsgbyid(NO)));
	disp_putstring(242 + DISP_FONTSIZE, 135 + 6 * DISP_FONTSIZE, COLOR_WHITE,
...
void scene_locsave_predraw(p_win_menuitem item, dword index, dword topindex,
						   dword max_height)
{
	disp_rectangle(237 - DISP_FONTSIZE * 10, 122 - 5 * DISP_FONTSIZE,
				   241 + DISP_FONTSIZE * 10, 136 + 6 * DISP_FONTSIZE,
				   COLOR_WHITE);
	disp_fillrect(238 - DISP_FONTSIZE * 10, 123 - 5 * DISP_FONTSIZE,
				  240 + DISP_FONTSIZE * 10, 122 - 4 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(238 - DISP_FONTSIZE * 3, 123 - 5 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) "保存文件位置");
	disp_line(238 - DISP_FONTSIZE * 10, 123 - 4 * DISP_FONTSIZE,
			  240 + DISP_FONTSIZE * 10, 123 - 4 * DISP_FONTSIZE, COLOR_WHITE);
}
...
dword scene_locsave(dword * selidx)
{
	...
	index =
		win_menu(238 - DISP_FONTSIZE * 10, 124 - 4 * DISP_FONTSIZE, 40,
				 NELEMS(item), item, NELEMS(item), 0, 0,
				 config.usedyncolor ? GetBGColorByTime() : config.menubcolor,
		...
		case PSP_CTRL_CIRCLE:
			{
				...
				if (*index == 0) {
					...
					// load
					if (!conf_load(&config)) {
						win_msg("读取设置失败!", COLOR_WHITE, COLOR_WHITE,
								config.msgbcolor);
						return win_menu_op_redraw;
					}
					...
					if (!conf_save(&config)) {
						win_msg("保存设置失败!", COLOR_WHITE, COLOR_WHITE,
								config.msgbcolor);
						return win_menu_op_redraw;
					}
				} else {
					...
					if (!utils_del_file(conffile)) {
						win_msg("删除设置失败!", COLOR_WHITE, COLOR_WHITE,
								config.msgbcolor);
						return win_menu_op_redraw;
					}
				}
...
{
#ifdef _DEBUG
	disp_rectangle(237 - DISP_FONTSIZE * 3, 120 - 7 * DISP_FONTSIZE,
				   241 + DISP_FONTSIZE * 3, 139 + 9 * DISP_FONTSIZE,
				   COLOR_WHITE);
#else
	disp_rectangle(237 - DISP_FONTSIZE * 3, 120 - 7 * DISP_FONTSIZE,
				   241 + DISP_FONTSIZE * 3, 138 + 8 * DISP_FONTSIZE,
				   COLOR_WHITE);
#endif
	disp_fillrect(238 - DISP_FONTSIZE * 3, 121 - 7 * DISP_FONTSIZE,
				  240 + DISP_FONTSIZE * 3, 120 - 6 * DISP_FONTSIZE,
				  config.usedyncolor ? GetBGColorByTime() : config.menubcolor);
	disp_putstring(238 - DISP_FONTSIZE * 2, 121 - 7 * DISP_FONTSIZE,
				   COLOR_WHITE, (const byte *) getmsgbyid(SETTING_OPTION));
	disp_line(238 - DISP_FONTSIZE * 3, 121 - 6 * DISP_FONTSIZE,
			  240 + DISP_FONTSIZE * 3, 121 - 6 * DISP_FONTSIZE, COLOR_WHITE);
}
...
dword scene_options(dword * selidx)
{
	...
	while ((index =
			win_menu(238 - DISP_FONTSIZE * 3, 122 - 6 * DISP_FONTSIZE, 12,
					 NELEMS(item), item, NELEMS(item), index, 0,
					 config.usedyncolor ? GetBGColorByTime() : config.
	...
	switch (key) {
		...
		case PSP_CTRL_START:
			if (win_msgbox
				...
				win_msg("已导出书签!", COLOR_WHITE, COLOR_WHITE,
						config.msgbcolor);
			}
			return win_menu_op_force_redraw;
		case PSP_CTRL_SQUARE:
							...
							dword max_height)
{
	...
	disp_putstring(75, 61 - DISP_FONTSIZE, COLOR_WHITE,
				   (const byte *) "书签      ○读取  ×取消  □保存  △删除");
	...
	disp_putstring(64, 65 + (1 + DISP_FONTSIZE) * 9, COLOR_WHITE,
				   (const byte *) "SELECT 删除全部书签    START 导出书签");
...
static bool confirm_overwrite(const char *filename, void *dummy)
{
	...
	SPRINTF_S(infomsg, "是否覆盖文本%s？", filename);
									...
									dword * topindex, dword * index)
{
	...
	if (key == (PSP_CTRL_SELECT | PSP_CTRL_START)) {
		...
		*index = 0;
		...
		disp_rectangle(240 - DISP_FONTSIZE * 3 - 1, 136 - DISP_FONTSIZE * 3 - 1,
					   240 + DISP_FONTSIZE * 3, 136 + DISP_FONTSIZE * 5,
					   COLOR_WHITE);
		disp_fillrect(240 - DISP_FONTSIZE * 3, 136 - DISP_FONTSIZE * 3,
					  240 + DISP_FONTSIZE * 3 - 1, 136 + DISP_FONTSIZE * 5 - 1,
					  config.msgbcolor);
			...
			switch ((t_fs_filetype) item[selidx].data) {
				...
				default:
					...
					disp_putstring(240 - DISP_FONTSIZE * 3, 136 - DISP_FONTSIZE,
								   COLOR_WHITE, (const byte *) "□  阅读文本");
					break;
			}
		} else {
			if (mp3count + dircount > 0 && bmcount > 0) {
				disp_putstring(240 - DISP_FONTSIZE * 3, 136 - DISP_FONTSIZE * 2,
							   COLOR_WHITE, (const byte *) "○  添加音乐");
				disp_putstring(240 - DISP_FONTSIZE * 3, 136 - DISP_FONTSIZE,
							   COLOR_WHITE, (const byte *) "□  导入书签");
		...
		if (where == scene_in_dir) {
			...
			if (strnicmp(config.path, "ms0:/", 5) == 0
				...
				disp_putstring(240 - DISP_FONTSIZE * 3, 136 + DISP_FONTSIZE * 3,
							   COLOR_WHITE, (const byte *) "START 粘贴");
		}
		if (where == scene_in_zip || where == scene_in_chm
			|| where == scene_in_rar) {
			if (selcount > 1 || strcmp(item[selidx].compname->ptr, "..") != 0) {
						...
						for (sidx = 0; sidx < filecount; sidx++)
							if (item[sidx].selected) {
								...
								if ((t_fs_filetype) item[sidx].data ==
									fs_filetype_dir) {
									if (!utils_del_dir(fn)) {
										win_msg("删除目录失败!", COLOR_WHITE,
												COLOR_WHITE, config.msgbcolor);
										return win_menu_op_redraw;
									}
								} else {
									if (!utils_del_file(fn)) {
										win_msg("删除文件失败!", COLOR_WHITE,
												COLOR_WHITE, config.msgbcolor);
										return win_menu_op_redraw;
									}
								}
				...
				case PSP_CTRL_SELECT:
					{
						...
						if (exif_array && exif_array->used) {
							...
							win_msg("无EXIF信息", COLOR_WHITE, COLOR_WHITE,
									config.msgbcolor);
					}
					break;
			}
							...
							dword max_height)
{
	...
	disp_putnstring(240 - WRR * DISP_FONTSIZE,
					...
					(const byte *) config.path, 40, 0, 0, DISP_FONTSIZE, 0);
...
void exec_homebrew(int method, char *path)
{
	...
	if ((err = xrKernelLoadExecVSHMsX(method, path, &param)) < 0) {
		...
		SPRINTF_S(infomsg, "%s: 方法%d启动错误, 返回错误代码: %08x", path,
				  method, err);
			...
			case fs_filetype_prog:
				{
					...
					SPRINTF_S(infomsg, "是否以%s方式执行该程序?",
							  config.launchtype == 2 ? "普通程序" : "PS游戏");
					if (win_msgbox
